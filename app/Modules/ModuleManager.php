...
class ModuleManager
{
    ...
    protected function loadModules(): void
    {
        if (config('modules.cache', true) && ! config('modules.development', false)) {
            try {
                ...
                \Log::debug('Module cache unavailable, loading from filesystem: '.$e->getMessage());
            }
        }

        // Load from app/Modules directory
        ...
        // Load from app-modules directory (modular pattern)
        ...
        // Load from external paths configured in modules.php
        foreach (config('modules.external_paths', []) as $externalPath) {
            if (is_string($externalPath) && File::isDirectory($externalPath)) {
        ...
        if (config('modules.cache', true) && ! config('modules.development', false)) {
            try {
                ...
                \Log::debug('Failed to cache module list: '.$e->getMessage());
            }
        }
    }
    ...
    public function meetsRequirements(ModuleInterface $module): bool
    {
        $requirements = config('modules.requirements', []);
    ...
    public function getHealthStatus(): array
    {
        return $this->modules->map(function (ModuleInterface $module) {
            return [
                ...
                'version' => $module->getVersion(),
                'enabled' => $module->isEnabled(),
                'meets_requirements' => $this->meetsRequirements($module),
            ];
        })->values()->toArray();
    }
