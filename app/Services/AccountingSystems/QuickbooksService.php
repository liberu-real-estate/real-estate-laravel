...
class QuickbooksService implements AccountingSystemInterface
{
    protected $apiKey;
    protected $endpoint;
    ...
    {
        $this->apiKey = config('services.quickbooks.api_key');
        $this->endpoint = config('services.quickbooks.endpoint');
    }

    public function syncInvoice(Invoice $invoice): bool
    {
        // Implement Quickbooks-specific invoice sync logic
        try {
    ...
    {
        // Implement Quickbooks-specific payment sync logic
        try {
            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . $this->apiKey,
                'Content-Type' => 'application/json',
